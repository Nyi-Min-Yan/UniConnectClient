...
import { getDepartmentStats, MAJORS, MAJOR_LABELS } from "@/lib/data";
import { ALL_USERS } from "@/lib/users";
import { getStudents, addStudent, type NewStudentInput } from "@/lib/store";
...
export default function StudentsPage() {
  ...
  const students = useMemo(() => getStudents(), [rerender]);
  const deptStats = useMemo(() => getDepartmentStats(), []);

  const majors = useMemo(() => [...new Set(students.map((s) => s.major))].sort(), [students]);

  const filtered = useMemo(() => {
    return students.filter((s) => {
      if (semester !== "all" && s.semester !== semester) return false;
      if (major !== "all" && s.major !== major) return false;
      if (year !== "all" && Math.ceil(s.semester / 2) !== year) return false;
      if (search) {
  ...
  const [form, setForm] = useState<NewStudentInput>({
    name: "", rollNo: "", semester: 1, email: "", major: "CS",
  });

  const handleAdd = () => {
    if (!form.name || !form.rollNo || !form.email) return;
    addStudent(form);
    setShowModal(false);
    setForm({ name: "", rollNo: "", semester: 1, email: "", major: "CS" });
    setRerender((n) => n + 1);
  };
    ...
    <div className="space-y-4 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-base-content/50 mt-0.5">{students.length} total &middot; {majors.length} majors</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {MAJORS.map((m) => {
          ...
          return (
            ...
            >
              <p className="text-[10px] font-bold text-base-content/50 uppercase tracking-wider">{MAJOR_LABELS[m] || m}</p>
              <p className="text-2xl font-bold text-base-content mt-1">{count}</p>
              <p className="text-[10px] text-base-content/40 mt-0.5">{Math.round((count / students.length) * 100)}% of total</p>
            </div>
          );
        })}
      </div>
      ...
      {filtered.length === 0 ? (
              ...
              <tbody className="divide-y divide-base-200">
                {filtered.map((student, idx) => (
                  <tr key={student.id} className="hover:bg-base-200/30 transition-colors">
                    ...
                    <td className="px-4 py-2.5 font-medium text-base-content">{student.name}</td>
                    <td className="px-4 py-2.5">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${majorColors[student.major]}`}>{MAJOR_LABELS[student.major]}</span>
                    </td>
                    <td className="px-4 py-2.5 text-center text-xs text-base-content/50">{student.semester}</td>
        ...
        sortedKeys.map((key) => {
          ...
          const subInfo = isDeptView
            ...
            : `${group.length} students &middot; ${[...new Set(group.map((s) => MAJOR_LABELS[s.major]))].join(", ")}`;
                  ...
                  <div key={student.id} className="flex items-center gap-3 px-4 py-3 hover:bg-base-200/30 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-base-content/50">
                        ...
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${majorColors[student.major]}`}>{MAJOR_LABELS[student.major]}</span>
                        <span className="w-1 h-1 rounded-full bg-base-content/20" />
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {isDeptView && (
                        <span className="text-[10px] text-base-content/40 bg-primary/10 px-2 py-0.5 rounded-full">
                          Yr {Math.ceil(student.semester / 2)}
                        </span>
                      )}
                    </div>
                  </div>
        ...
        <ModalPortal onClose={() => setShowModal(false)}>
          <div className="bg-base-100 rounded-2xl shadow-2xl border border-base-200 p-5 w-full animate-fade-in-up">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-bold text-base-content">Add Student</h2>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Maung Maung" className="w-full px-3 py-2.5 text-sm rounded-xl border border-base-200 bg-base-100 text-base-content outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 placeholder:text-base-content/30" />
              </div>
              <div>
                <label className="text-xs font-medium text-base-content/60 mb-1 block">Roll Number</label>
                <input type="text" value={form.rollNo} onChange={(e) => setForm({ ...form, rollNo: e.target.value })} placeholder="e.g. UCSTGO-2501" className="w-full px-3 py-2.5 text-sm rounded-xl border border-base-200 bg-base-100 text-base-content outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 placeholder:text-base-content/30" />
              </div>
              <div>
                <label className="text-xs font-medium text-base-content/60 mb-1 block">Email</label>
                <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="e.g. maung.maung@uni.edu" className="w-full px-3 py-2.5 text-sm rounded-xl border border-base-200 bg-base-100 text-base-content outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 placeholder:text-base-content/30" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-base-content/60 mb-1 block">Major</label>
                  <select value={form.major} onChange={(e) => setForm({ ...form, major: e.target.value as "CS" | "CT" | "CST" })} className="w-full px-3 py-2.5 text-sm rounded-xl border border-base-200 bg-base-100 text-base-content outline-none focus:border-primary">
                    {MAJORS.map((m) => (
                      <option key={m} value={m} disabled={m === "CST" && form.semester > 2}>
                        {MAJOR_LABELS[m] || m}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <select value={form.semester} onChange={(e) => setForm({ ...form, semester: Number(e.target.value) })} className="w-full px-3 py-2.5 text-sm rounded-xl border border-base-200 bg-base-100 text-base-content outline-none focus:border-primary">
                    {Array.from({ length: 8 }, (_, i) => (
                      <option key={i + 1} value={i + 1}>Semester {i + 1}</option>
                    ))}
                  </select>
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button onClick={handleAdd} disabled={!form.name || !form.rollNo || !form.email} className="flex-1 px-4 py-2.5 text-sm font-bold rounded-xl bg-primary text-white hover:bg-primary/90 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
