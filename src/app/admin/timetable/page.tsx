...
// Helper to get course code from course object
...
export default function TimetablePage() {
  ...
  // Build timetable from schedules
  useEffect(() => {
    if (
      schedules &&
      courses &&
  ...
  const handleGenerate = async () => {
    if (!activeTerm) {
      setToast({ visible: true, message: "No active term found", type: "error" });
      return;
    }

    const generator = staff?.[0];
    if (!generator) {
      setToast({ visible: true, message: "No staff member available to generate timetable", type: "error" });
      return;
    }

    setGenerating(true);
    try {
      const created = await apiClient.post("/api/generations", {
        termId: activeTerm.termId,
        generatedByStaffId: generator.staffId,
      });
      const generation = created.data;

      await apiClient.post(`/api/generations/${generation.generationId}/generate`);
      await apiClient.post(`/api/generations/${generation.generationId}/publish`);

      setSelectedGenerationId(generation.generationId);

      setToast({
        visible: true,
        message: `Timetable generated successfully for ${activeTerm.academicYear}`,
        type: "success",
      });
    } catch (error) {
      setToast({
        visible: true,
        message: error instanceof Error ? error.message : "Failed to generate timetable",
        type: "error",
      });
    } finally {
      setGenerating(false);
          ...
          <div>
            ...
            <p className="text-white/70 text-sm mt-0.5">
              {publishedGeneration
                ...
                : "Auto-schedule — teacher conflict-free"}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          ...
          <div className="flex items-end gap-1">
            <button
              ...
              className="flex-1 px-4 py-2 text-sm font-bold rounded-xl bg-primary text-white hover:bg-primary/90 transition-all shadow-sm disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {generating ? (
                <>
                ...
                {teacherTimetable.length > 0 ? (
                  ...
                  <div className="bg-base-100 rounded-2xl border border-base-200 p-8 text-center">
                    <p className="text-sm text-base-content/50">
                      No timetable generated yet. Generate one first.
                    </p>
                  </div>
                )}
              </>
        ...
        <div className="bg-base-100 rounded-2xl border border-base-200 p-5 shadow-sm">
          ...
          {currentTimetable && currentTimetable.entries.length > 0 ? (
            ...
            <p className="text-xs text-base-content/40 text-center py-8">
              Generate a timetable to see LMS & Assignment links
            </p>
          )}
        </div>
      )}
